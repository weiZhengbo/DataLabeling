				...
				<div>
					<form style="float:left">
						<c:choose>
							...
							<c:otherwise>
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<input type="radio" name="data" value="all" onclick="getUrl('dataType','all')" checked/>已标注数据
								&nbsp;&nbsp;&nbsp;&nbsp;
								...
								<div class="dropdown" >
									...
									<ul class="dropdown-menu" aria-labelledby="dropdownMenu1" style="height:200px;overflow:scroll;margin-left: 25%">
										<c:forEach items="${pb.TClasses }" var="similarClass">
											<li onclick="changeChoosed(this)" id="li${similarClass.id}" aria-checked="true"><a>${similarClass.recordClass}</a></li>
										</c:forEach>
									</ul>
								</div>
							</div>
								...
								<table class="table table-hover similarTag ">
									...
									<c:forEach items="${pb.TClasses }" var="similarClass">
										<tr draggable="true" ondragstart="drag(event)" id="sid${similarClass.id}">
											...
											<td onclick="getUrl('clickwordId',${similarClass.id})">${similarClass.recordClass}</td>
											<td onclick="similarTagDelete(${similarClass.id},'${similarClass.recordClass}')">
						...
						<c:otherwise>
							<div class="table-content" style="border: 1px solid #eee;height: 300px;overflow-y: scroll;overflow-x: hidden;">
								<table class="table table-hover similarTag colorFulTag">
									...
									<c:forEach items="${pb.TClasses }" var="similarClass">
										<tr draggable="true" ondragstart="drag(event)" id="sid${similarClass.id}">
											<td onclick="changeStatus('clickwordId',${similarClass.id})">${similarClass.id}</td>
											<td onclick="changeStatus('clickwordId',${similarClass.id})">${similarClass.recordClass}</td>
											<td onclick="similarTagDelete(${similarClass.id},'${similarClass.recordClass}')">
												<i class='glyphicon glyphicon-remove' ></i>
											</td>
										</tr>
...
<script type="application/javascript" src="./js/listRecord.js"></script>
</body>
</html>
